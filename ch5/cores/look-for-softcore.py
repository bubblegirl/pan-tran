...
#use soft-/core file to read line by line and find cluster
	with open('softcore-cluster-IDs.txt', 'r') as corey:
		for ping in corey:
			pingstrip = ping.strip()
			print pingstrip
#make new file with contig ID plus _core
			thingy = open(pingstrip + '_softcore.txt', 'w')
#print line ID
#			thingy.write(ping)
#			thingy.write("\n")
...
def curatecores():
	for file in glob('*_softcore.txt'):
		named = file.rsplit("_", 1)[0]
		outtie = open(named + '_softcore-curated.txt', 'w')
...
def fundicts(): #um... build a fucking dictionary for GO terms and then see what is the most common in each file, look that up
	registry = open('Softore-clusters-registry.txt', 'w')
	registry.write("file-name\tGOid;GOcount\ttotal-contigs\tcontigs-per-species")
	registry.write("\n")
	for file in glob('*_softcore-curated.txt'):
